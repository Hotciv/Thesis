...
# # A little test to see if our kernel output matrices of the correct shape
# x_len, y_len = 5, 10
# print(linear_kernel(np.random.rand(x_len, 1), np.random.rand(y_len, 1)).shape == (x_len,y_len))
# # True
# print(gaussian_kernel(np.random.rand(x_len, 1), np.random.rand(y_len, 1)).shape == (5,10))
# # True
...
def plot_decision_boundary(
    model, ax, resolution=100, colors=("b", "k", "r"), levels=(-1, 0, 1)
):
  ...
  Returns decision boundary grid and axes object (`grid`, `ax`)."""
    ...
    ax.scatter(
        model.X[:, 0], model.X[:, 1], c=model.y, cmap=plt.cm.viridis, lw=0, alpha=0.25
    )
...
# Instantiate model
model = SMOModel(
    X_train_scaled, y, C, linear_kernel, initial_alphas, initial_b, np.zeros(m)
)

# Initialize error cache
initial_error = (
    decision_function(model.alphas, model.y, model.kernel, model.X, model.X, model.b)
    - model.y
)
...
plt.plot(decision_function(model.alphas, model.y, model.kernel, model.X, model.X, model.b))
plt.show()
...
# Instantiate model
model = SMOModel(
    X_outlier, y_outlier, C, linear_kernel, initial_alphas, initial_b, np.zeros(m)
)
...
# Instantiate model
model = SMOModel(
    X_train_scaled, y, C, gaussian_kernel, initial_alphas, initial_b, np.zeros(m)
)
